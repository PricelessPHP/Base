...
class Email
{
    ...
    public function send( $params = array() )
    {
        ...
        $this->_PHPMailer->From		= trim( @$params['email']['from']['email'] );
        $this->_PHPMailer->FromName = ( strlen( trim( @$params['email']['from']['name'] ) ) > 0 ) ? trim( $params['email']['from']['name'] ) : $params['email']['from']['email'];
